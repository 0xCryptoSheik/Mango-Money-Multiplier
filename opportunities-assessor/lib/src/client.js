    ...
    sendTransaction(transaction, payer, additionalSigners, timeout = 30000, confirmLevel = 'processed') {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (err.timeout) {
                    throw new src_1.TimeoutError({ txid });
                }
                ...
                throw new src_1.MangoError({ message: 'Transaction failed', txid });
            }
            finally {
                done = true;
            }
            ...
            for (let i = 0; i < mangoAccount.spotOpenOrders.length; i++) {
                ...
                if (i === spotMarketIndex) {
                    ...
                    if (mangoAccount.spotOpenOrders[spotMarketIndex].equals(utils_1.zeroKey)) {
                        ...
                        const initOpenOrders = src_1.makeCreateSpotOpenOrdersInstruction(this.programId, mangoGroup.publicKey, mangoAccount.publicKey, owner.publicKey, mangoGroup.dexProgramId, openOrdersPk, spotMarket.publicKey, mangoGroup.signerKey);
                        initTx.add(initOpenOrders);
    ...
    getReferrerIdsForMangoAccount(mangoAccount) {
        return __awaiter(this, void 0, void 0, function* () {
            const filters = [
                {
                    memcmp: {
                        offset: src_1.ReferrerIdRecordLayout.offsetOf('referrerMangoAccount'),
                        bytes: mangoAccount.publicKey.toBase58(),
                    },
                },
                {
                    dataSize: src_1.ReferrerIdRecordLayout.span,
                },
            ];
            const referrerIds = yield utils_1.getFilteredProgramAccounts(this.connection, this.programId, filters).then((referrerIds) => {
                return referrerIds.map(({ accountInfo }) => {
                    return new src_1.ReferrerIdRecord(src_1.ReferrerIdRecordLayout.decode(accountInfo == null ? undefined : accountInfo.data));
                });
            });
            return referrerIds;
        });
